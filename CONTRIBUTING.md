...
that way. The binary gets installed on routers with 8 MiB of flash,
`CGO_ENABLED=0` so it links against nothing at all, and the whole point is that
it keeps working after the machine has been left alone for a year. A dependency
has to earn a lot to be worth losing that.
...
than which files were touched. The body explains why, wrapped at 72. One change
per commit.
