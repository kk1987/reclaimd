...
export const DICT = {
  zh: {
    ...
    'reason.ONE_SCSI_COMMAND': '这支盘的 max_sectors_kb 是 {max_sectors_kb} KiB，所以一次读恰好是一条 SCSI 命令。',
    ...
    'disk.scanQueued': '已请求，等下一轮调度开始（最多 30 秒）',
    'disk.scanNeedsMaintain': '已排除，先纳入维护才能扫',
    'disk.maintainAction': '当前：已排除。点击纳入维护',
    'disk.excludeAction': '当前：纳入维护。点击排除（不打断正在跑的一趟）',
    'disk.forget': '删除这支盘的全部记录',
    'disk.forgetScanning': '正在扫描，扫完再删',
    'foot.method': '读取走 O_DIRECT 直达裸设备：走页缓存读到的是缓存，什么也刷新不了。设备按 sysfs 里的 USB 序列号识别，掉线改名后自动重新绑定。',
  ...
  en: {
    ...
    'reason.ONE_SCSI_COMMAND': 'This drive’s max_sectors_kb is {max_sectors_kb} KiB, so one read is exactly one SCSI command.',
    ...
    'disk.scanQueued': 'Requested; the round starts on the next tick, within 30s',
    'disk.scanNeedsMaintain': 'Excluded; maintain it before scanning',
    'disk.maintainAction': 'Excluded. Click to maintain',
    'disk.excludeAction': 'Maintained. Click to exclude (does not stop a running pass)',
    'disk.forget': 'Delete everything stored for this disk',
    'disk.forgetScanning': 'Scanning; it can be deleted once the round ends',
    'foot.method': 'Reads go through O_DIRECT straight to the raw device: a buffered read would come from cache and refresh nothing. Devices are identified by their USB serial in sysfs, so a rename after a dropout rebinds automatically.',
