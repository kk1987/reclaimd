...
export const DICT = {
  zh: {
    ...
    'rewrite.title': '重写这支盘',
    'rewrite.lede': '读只能治好主控自己判定为边缘的块。重写会把每一块都过一遍新的编程周期，不管主控怎么判定。这个页面不执行重写，只把命令拼好。守护进程始终只读，写盘是另一条命令的事。',
  ...
  en: {
    ...
    'rewrite.lede': 'Reading only heals the blocks the controller judges marginal. Rewriting puts every block through a fresh program cycle, whatever the controller makes of it. This page only assembles the command and never runs it. The daemon stays read-only, and writing is a separate command.',
