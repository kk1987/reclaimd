...
export const DICT = {
  zh: {
    ...
    'structure.stubbornNote': '连续多轮偏慢、且从未回探成功的段。99% 的退化块读一次就自愈，剩下的才值得看——考虑对它们跑 reclaimd refresh -range=。',
    'structure.stubbornNone': '没有顽固区。',
    'structure.streak': '连续 {n} 轮',

    'rewrite.title': '重写这支盘',
    'rewrite.lede': '读只能治好主控自己判定为边缘的块；重写会把每一块都过一遍新的编程周期，无论主控同不同意。这个页面不执行重写，只把命令拼好——守护进程始终只读，写盘是另一条命令的事。',
    'rewrite.dryLabel': '先空跑，确认瞄准的是这支盘',
    'rewrite.runLabel': '确认无误后，真正重写',
    'rewrite.copy': '复制',
    'rewrite.copied': '命令已复制',
    'rewrite.copyFail': '复制不了，请手动选中命令',
    'rewrite.gates': '在能插到这支盘的那台机器上跑。跑之前先卸载它的所有分区——命令自己也会拦，但先卸载省得白跑一趟。整盘重写要按盘速跑上几十分钟到几小时。',
  ...
  en: {
    ...
    'rewrite.lede': 'Reading only heals the blocks the controller judges marginal; rewriting puts every block through a fresh program cycle whether it agrees or not. This page does not run it — it only assembles the command. The daemon stays read-only, and writing is a separate command.',
    'rewrite.dryLabel': 'Dry run first, to confirm it is aimed at this drive',
    ...
    'rewrite.gates': 'Run it on the machine this drive is plugged into. Unmount every partition of it first — the command refuses otherwise, but unmounting first saves a wasted trip. A whole-drive rewrite takes tens of minutes to hours at this drive’s speed.',
    ...
    'structure.caption': 'Worst latency grouped by offset mod {seg}. The peak sits at offset {peak} ({peakMs}) \u2014 the last position in every erase block, the most fragile wordline on the die.',
    ...
    'structure.stubbornNote': 'Segments that read slow for several passes running and never came back healed. 99% of degraded blocks fix themselves on the next read; these are the rest \u2014 consider reclaimd refresh -range= over them.',
    ...
    'disk.identBy': 'This drive\u2019s serial \u2014 what tells two identically named sticks apart',
    ...
    'disk.stopAction': 'Scanning. Click to stop this pass; it picks up where it stopped in an hour',
    ...
    'disk.forgetScanning': 'Scanning; it can be deleted once the round ends',
    ...
    'toast.forgotten': 'Deleted; the disk is no longer on the list',
